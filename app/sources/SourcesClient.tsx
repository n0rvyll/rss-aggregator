...
export default function SourcesClient() {
  ...
  // kezdeti kiválasztás URL-ből (ha van), különben üres -> majd töltés után all
  ...
  const [showSport, setShowSport] = useState<boolean>(() => {
    ...
    try {
      return localStorage.getItem("showSport") !== "0";
    } catch {
      return true;
    }
  });
  const [showTabloid, setShowTabloid] = useState<boolean>(() => {
    ...
    try {
      return localStorage.getItem("showTabloid") === "1";
    } catch {
      return false;
    }
  });

  // Források betöltése + selected inicializálása
  useEffect(() => {
    let alive = true;
    (async () => {
      setLoading(true);
      ...
      try {
        ...
        // ha URL-ben nincs semmi → MIND
        if (initSelected.size === 0) {
          setSelected(new Set(list.map((x) => x.id)));
        } else {
          // ha URL-ben vannak id-k, de nem mind létezik → csak a meglévőket
          const onlyExisting = new Set(
            list.filter((x) => initSelected.has(x.id)).map((x) => x.id)
          );
          setSelected(
            onlyExisting.size ? onlyExisting : new Set(list.map((x) => x.id))
          );
        }
      } catch (e) {
        if (alive) {
          const msg =
            e instanceof Error ? e.message : typeof e === "string" ? e : "Hiba";
          setErr(msg);
        }
      } finally {
  ...
  useEffect(() => {
    try {
      localStorage.setItem("showSport", showSport ? "1" : "0");
    } catch {}
  }, [showSport]);
  useEffect(() => {
    try {
      localStorage.setItem("showTabloid", showTabloid ? "1" : "0");
    } catch {}
  }, [showTabloid]);
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-sm text-neutral-500">
              Válaszd ki, mely forrásokat szeretnéd látni.
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title={
                allSelected ? "Összes kikapcsolása" : "Összes bekapcsolása"
              }
            >
              {allSelected ? "◯" : "●"}
            </button>

...
/* ---------------------- Settings tartalom (újrahasznosítva) ---------------------- */
...
/* ---------------------- SettingsSheet (mobil bottom sheet, desktop drawer) ---------------------- */
...
export default function Page() {
  ...
  /* ---------- HYDRATION-SAFE alapállapotok, majd mount után betöltés ---------- */
  const [theme, setTheme] = useState<"system" | "light" | "dark">("system");
  const [dense, setDense] = useState<boolean>(false);
  const [masonry, setMasonry] = useState<boolean>(false);
  ...
  // csak kliensen olvassuk be a perzisztált értékeket (így nincs mismatch)
  useEffect(() => {
    try {
      const t =
  ...
  // téma DOM-ra alkalmazása + mentés
  useEffect(() => {
  ...
  // --- Mobil fejléc elrejtés/görgetés ---
  ...
  // ha bármelyik menü nyitva van, ne rejtsük el
  useEffect(() => {
    if (menuOpen || settingsOpen) setHideHeader(false);
  }, [menuOpen, settingsOpen]);

  // helper: URL frissítés
  ...
  /* ------- Forrásválasztó (mobil natív select + desktop popover) ------- */
    ...
    <main className="min-h-screen bg-gradient-to-b from-neutral-50 to-neutral-100 dark:from-neutral-900 dark:to-neutral-950">
      <div className="mx-auto max-w-[90rem] p-5 md:p-8 space-y-7">
        ...
        >
          ...
          <nav className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              {/* MOBIL: natív select a forrásokhoz */}
        ...
        <footer className="py-8 text-center text-xs text-neutral-500">
          /// valami szöveg kell ide ///
        </footer>
      </div>

      {/* bal alsó értesítő az új cikkekről */}
      <div
        className={[
      ...
      {/* Kereső kompakt + globális apróságok */}
      <style jsx global>{`
